...
const Requests = () => {
  ...
  const reviewRequest=async(status,_id)=>{
    try{//passed empty because here we are not sending any typed request
      ...
      {requests.map((e) => {
        ...
        return (
          ...
          >
            ...
            <div>
              <button className="btn btn-primary mx-2" onClick={()=>reviewRequest("rejected",e._id)}>Reject</button>
              <button className="btn btn-secondary mx-2 " onClick={()=>reviewRequest("accepted",e._id)}>Accept</button>
            </div>
          </div>
        );
      })}
